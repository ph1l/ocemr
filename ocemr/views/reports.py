...
def lab_tally(request):
    ...
    dt_start = datetime(date_start_in.year, date_start_in.month,
                        date_start_in.day, 0, 0, 0)
    dt_end = datetime(date_end_in.year, date_end_in.month, date_end_in.day, 23,
                      59, 59)
    q_this_day = (Q(orderedDateTime__gte=dt_start) & Q(
        orderedDateTime__lte=dt_end)) & (Q(status="CAN") | Q(status="COM")
                                         | Q(status="FAI"))
    ...
    if dump_type == "G_PIE":
        title = "Lab Tally %s -> %s" % (dt_start.strftime("%Y-%m-%d"),
    ...
    if dump_type == "CSV":
        return dump_csv(
            "lab-tally-%s-%s.csv" % (dt_start.strftime("%Y%m%d"),
                                     dt_end.strftime("%Y%m%d")), field_names,
...
def med_tally(request):
    ...
    if dump_type == "G_PIE":
        title = "Med Tally %s -> %s" % (dt_start.strftime("%Y-%m-%d"),
                                        dt_end.strftime("%Y-%m-%d"))
        labels = []
        data = []
        for key in sorted_keys:
    ...
    if dump_type == "CSV":
        return dump_csv(
            "med-tally-%s-%s.csv" % (dt_start.strftime("%Y%m%d"),
                                     dt_end.strftime("%Y%m%d")), field_names,
            headers, summary_rows)
    elif dump_type == "TABLE":
        return dump_table(request, field_names, headers, summary_rows)
...
def village_tally(request):
    ...
    for village in sorted_keys:
        summary_rows.append({
            'village': village,
            'num_patients': totals[village]
        })
    if dump_type == "CSV":
        return dump_csv(
            "village-tally-%s.csv" % (datetime.now().strftime("%Y%m%d")),
            field_names, headers, summary_rows)
    elif dump_type == "TABLE":
        return dump_table(request, field_names, headers, summary_rows)
...
def clinician_tally(request):
    ...
    dt_end = datetime(date_end_in.year, date_end_in.month, date_end_in.day, 23,
                      59, 59)
    q_this_day = (Q(finishedDateTime__gte=dt_start) & Q(
        finishedDateTime__lte=dt_end)) & (Q(status="CHOT") | Q(status="RESO"))
    ...
    for clinician in sorted_keys:
        summary_rows.append({
            'clinician': clinician,
            'num_patients': totals[clinician]
        })
    if dump_type == "CSV":
        return dump_csv(
            "clinician-visit-tally-%s-%s.csv" % (dt_start.strftime("%Y%m%d"),
                                                 dt_end.strftime("%Y%m%d")),
            field_names, headers, summary_rows)
    elif dump_type == "TABLE":
        return dump_table(request, field_names, headers, summary_rows)
...
def diagnosis_tally(request):
    ...
    if dump_type == "G_PIE":
        title = "Diagnosis Tally %s -> %s" % (dt_start.strftime("%Y-%m-%d"),
                                              dt_end.strftime("%Y-%m-%d"))
        labels = []
        data = []
        for key in sorted_keys:
    ...
    if dump_type == "CSV":
        return dump_csv(
            "diagnosis-tally-%s-%s.csv" % (dt_start.strftime("%Y%m%d"),
                                           dt_end.strftime("%Y%m%d")),
            field_names, headers, summary_rows)
    elif dump_type == "TABLE":
        return dump_table(request, field_names, headers, summary_rows)
...
def legacy_patient_daily(request):
    ...
    field_names = [
        'pt_daily_index', 'pt_name', 'pt_monthly_index', 'sex', 'age',
        'village', 'diagnosis', 'prescription', 'referral'
    ]
    headers = {
    ...
    q_this_month = (Q(finishedDateTime__gte=dt_month_start) & Q(
        finishedDateTime__lt=dt_month_end) & Q(finishedDateTime__lt=dt_start)
    ...
    return dump_csv("patient-daily-%s.csv" % (date_in.strftime("%Y%m%d")),
                    field_names, headers, summary_rows)
...
def cashflow(request):
    ...
    return dump_csv(
        "cashflow-%s-%s.csv" % (date_start_in.strftime("%Y%m%d"),
                                date_end_in.strftime("%Y%m%d")), field_names,
        headers, summary_rows)
...
def accounts_outstanding(request):
    ...
    for v in Visit.objects.filter(
        ...
        for c in CashLog.objects.filter(visit=v):
            collected += c.amount
        if collected < v.cost:
            existing = filter(lambda person: person['patient'] == v.patient,
                              summary_rows)
            if existing:
    ...
    return dump_csv(
        "outstanding_accounts-%s-%s.csv" % (date_start_in.strftime("%Y%m%d"),
                                            date_end_in.strftime("%Y%m%d")),
        field_names, headers, summary_rows)
...
def diagnosis_patient(request):
    ...
    q = (Q(diagnosedDateTime__gte=dt_start) &
         Q(diagnosedDateTime__lte=dt_end)) & Q(status="NEW")
    ...
    if dump_type == "CSV":
        return dump_csv(
            "diagnosis-patient-%s-%s.csv" % (dt_start.strftime("%Y%m%d"),
                                             dt_end.strftime("%Y%m%d")),
            field_names, headers, summary_rows)
    elif dump_type == "TABLE":
        return dump_table(request, field_names, headers, summary_rows)
...
def hmis105(request):
    ...
    diagnoses = [
        ...
        {
            'NAME':
            "Other Emerging Infectious Diseases , specify e.g. small pox, ILI, SARS",
            'ICPC2': [],
        },
        {
        ...
        {
            'NAME':
            "22 Other Sexually Transmitted Infections",
            'ICPC2': [
                "X70", "X71", "X72", "X73", "X74", "X90", "X91", "X92", "Y70",
                "Y71", "Y72", "Y73", "Y74", "Y75", "Y76"
            ],
        },
        {
        ...
        {
            'NAME': "37 Neonatal Sepsis (0-7days)",
            'ICPC2': [],
            'MAX_AGE_DAYS': 7
        },
        {
        ...
        {
            'NAME':
            "46 Gastro-Intestinal Disorders (non-Infective) ",
            'ICPC2': [
                "D0", "D1", "D2", "D74", "D75", "D76", "D77", "D78", "D79",
                "D8", "D9"
            ],
        },
        {
        ...
        {
            'NAME':
            "55 Other ENT conditions",
            'ICPC2': [
                "H", "R06", "R07", "R08", "R09", "R1", "R20", "R21", "R72",
                "R73", "R87", "R90", "R97"
            ],
        ...
        {
            'NAME':
            "64 Other Eye conditions",
            'ICPC2': [
                "F",
            ],
            'subtract': [
                "56 Ophthalmia neonatorum", "57 Cataracts",
                "58 Refractive errors", "59 Glaucoma", "60 Trachoma",
                "61 Tumors", "62 Blindness", "63 Diabetic Retinopathy"
            ]
        },
        {
            'NAME': "Mental Health"
        ...
        {
            'NAME':
            "75 Other Mental Health Conditions",
            'ICPC2': [
        ...
        {
            'NAME':
            "91 Other Cardiovascular Diseases",
            'ICPC2': [
                "K",
            ],
            'subtract': [
        ...
        {
            'NAME':
            "98 Injuries- Road traffic Accidents (Manually review A80, A81)",
            'ICPC2': ["A80", "A81"],
        },
        {
            'NAME':
            "99 Injuries due to motorcycle(boda-boda)(Manually review A80, A81)",
            'ICPC2': ["A80", "A81"],
        },
        {
            'NAME':
            "100 Injuries due to Gender based violence (Manually review A80, A81)",
            'ICPC2': ["A80", "A81"],
        },
        {
            'NAME':
            "101 Injuries (Trauma due to other causes) (Manually review A80, A81)",
            'ICPC2': ["A80", "A81"],
        },
        {
        ...
        {
            'NAME':
            "108 Lymphatic Filariasis (hydrocele) (Manually review A78)",
            'ICPC2': [
                "A78",
            ],
        },
        {
            'NAME':
            "109 Lymphatic Filariasis (Lymphoedema) (Manually review A78)",
            'ICPC2': [
                "A78",
            ],
        },
        ...
        {
            'NAME':
            "120 Other diagnoses (specify priority diseases for District)",
            'ICPC2': [],
        },
        {
    ...
    for v in visits:
        ...
        if v.patient.birthDate:
            if v.patient.birthDate > (
                    v.finishedDateTime - timedelta(days=28)).date():
                index = 0
        ...
        if Visit.objects.filter(
                Q(patient=v.patient) & Q(finishedDateTime__gte=dt_start) & Q(
                    finishedDateTime__lte=dt_end)).count() > 1:
            old_patient_visits[index].append(v.patient.id)
        if Visit.objects.filter(
                Q(patient=v.patient) & Q(
                    finishedDateTime__lte=dt_start)).count() < 1:
    ...
    for d in diagnoses:
        if d.has_key("types") and len(d["types"]) == 0:
            ...
            summary_rows.append({
                ...
                diag_map[d['NAME']][9],
                'visit_list':
                ", ".join(diag_map[d['NAME']][10])
            })

    if dump_type == "CSV":
        return dump_csv(
            "hmis-105-%s-%s.csv" % (dt_start.strftime("%Y%m%d"),
                                    dt_end.strftime("%Y%m%d")), field_names,
            headers, summary_rows)
    elif dump_type == "TABLE":
        return dump_table(request, field_names, headers, summary_rows)
...
def med_pricelist(request):
    ...
    for m in MedType.objects.all().order_by('title'):
        data.append(
            [textwrap.fill(m.title, width=32, expand_tabs=False), m.cost])
    ...
    def header(canvas, doc):
        ...
        P = Paragraph("Engeye Medication Price List as of %s" % today,
                      styles['Heading1'])
