...
def get_backup(request):
    ...
    outfile = os.path.join(
        backup_dir,
        'backup_%s.%s' % (time.strftime('%y%m%d-%H%M%S'), DATABASE_ENGINE))
    try:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                return render(request, 'popup_lines.html',
                              {'lines': "ERROR: %s" % err})
            return HttpResponseRedirect('/close_window/')
    else:
...
def village_merge_wizard(request):
    ...
    out_txt = "Merge %s: %s\n  into %s: %s\n\n" % (
        villageIncorrect.id, villageIncorrect, villageCorrect.id,
        villageCorrect)
    ...
    out_link = "<A HREF=/village_merge_wizard/%d/%d/>Do the merge!</A> or " % (
        villageCorrect.id, villageIncorrect.id)
    return render(
