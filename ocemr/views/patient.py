...
def patient_queue(request, dayoffset=0):
    ...
    resolved_q = (Q(finishedDateTime__gte=dt_start) & Q(
...
def patient_new(request):
    ...
    if request.method == 'POST':  # If the form has been submitted...
        form = NewPatientForm(request.user,
...
def patient_edit_gender(request, id):
    ...
    if request.method == 'POST':  # If the form has been submitted...
        ...
        form = EditPatientGenderForm(initial={'gender':
                                              p.gender})  # An unbound form
    return render(
        request, 'popup_form.html', {
...
def patient_edit_phone(request, id):
    ...
    if request.method == 'POST':  # If the form has been submitted...
        ...
        form = EditPatientPhoneForm(initial={'phone':
                                             p.phone})  # An unbound form
    return render(
        request, 'popup_form.html', {
...
def patient_edit_email(request, id):
    ...
    if request.method == 'POST':  # If the form has been submitted...
        ...
        form = EditPatientEmailForm(initial={'email':
                                             p.email})  # An unbound form
    return render(
        request, 'popup_form.html', {
...
def patient_edit_age(request, id):
    ...
    if request.method == 'POST':  # If the form has been submitted...
        ...
        form = EditPatientAgeForm(initial={
...
def patient_edit_village(request, id):
    ...
    if request.method == 'POST':  # If the form has been submitted...
        ...
        form = EditPatientVillageForm(initial={'village':
                                               p.village})  # An unbound form
    return render(
        request, 'popup_form.html', {
...
def patient_edit_note(request, id):
    ...
    p = Patient.objects.get(pk=id)
    if request.method == 'POST':  # If the form has been submitted...
        form = EditPatientNoteForm(
            p, request.POST)  # A form bound to the POST data
        if form.is_valid():  # All validation rules pass
...
def patient_edit_alt_contact_name(request, id):
    ...
    if request.method == 'POST':  # If the form has been submitted...
        ...
        form = EditPatientAltContactNameForm(
            initial={'alt_contact_name': p.altContactName})  # An unbound form
    return render(
        request, 'popup_form.html', {
...
def patient_edit_alt_contact_phone(request, id):
    ...
    if request.method == 'POST':  # If the form has been submitted...
        ...
        form = EditPatientAltContactPhoneForm(
            initial={'alt_contact_phone':
                     p.altContactPhone})  # An unbound form
    return render(
        request, 'popup_form.html', {
...
def patient(request, id):
    ...
    p = Patient.objects.get(pk=id)
    upcoming_visits = Visit.objects.filter(patient=p).filter(
        status='SCHE').order_by('scheduledDate')
    visits = Visit.objects.filter(patient=p).exclude(
        status='SCHE').order_by('-seenDateTime')
        ...
        if form.is_valid():  # All validation rules pass
            ...
            if name != "":
                ...
                for term in name.split():
                    if i:
                        q_name = q_name & (Q(familyName__icontains=term)
                                           | Q(givenName__icontains=term))
                    else:
                        q_name = (Q(familyName__icontains=term)
                                  | Q(givenName__icontains=term))
                    i += 1
...
def schedule_new_visit(request, id):
    ...
    if request.method == 'POST':  # If the form has been submitted...
        form = NewScheduledVisitForm(
            request.user, p, request.POST)  # A form bound to the POST data
        if form.is_valid():  # All validation rules pass
            o = form.save()
            return HttpResponseRedirect('/close_window/')
    else:
...
def edit_visit(request, id):
    ...
    if request.method == 'POST':  # If the form has been submitted...
        form = EditScheduledVisitForm(
            v, request.user, request.POST)  # A form bound to the POST data
        if form.is_valid():  # All validation rules pass
...
def edit_visit_seen(request, id):
    ...
    if request.method == 'POST':  # If the form has been submitted...
        ...
        if form.is_valid():  # All validation rules pass
            v.seenDateTime = "%s %s" % (form.cleaned_data['seenDate'],
        ...
        form = EditVisitSeenForm(
            initial={
                'seenDate':
                formats.date_format(v.seenDateTime, 'SHORT_DATE_FORMAT'),
                'seenTime':
                formats.date_format(v.seenDateTime, 'TIME_FORMAT'),
            })
    return render(
        request, 'popup_form.html', {
...
def edit_visit_reason(request, id):
    ...
    v = Visit.objects.get(pk=id)
    if request.method == 'POST':  # If the form has been submitted...
        form = EditVisitReasonForm(
            v, request.POST)  # A form bound to the POST data
        if form.is_valid():  # All validation rules pass
            v.reasonDetail = form.cleaned_data['reasonDetail']
            v.save()
            return HttpResponseRedirect('/close_window/')
...
def patient_merge(request, id):
    ...
    out_link = "<A HREF=/patient/merge/%d/%d/>Do the merge!</A> or " % (
        p.id, pdup.id)
    return render(
        request, 'popup_info.html', {
            'title': 'Schedule Patient Visit',
