...
def vitals_height_weight(request, id):
    ...
    plt.plot(
        height_date_list, height_data_list, 'o-', color='r', label="Height")
    plt.plot(
        weight_date_list, weight_data_list, 'o-', color='m', label="Weight")
    plt.ylabel('cm, kg')
    plt.legend(loc=0)
    try:
        plt.xlim(
            matplotlib.dates.date2num(
                min(height_date_list + weight_date_list)) - 1,
            matplotlib.dates.date2num(
                max(height_date_list + weight_date_list)) + 1)
    except ValueError:
        pass
    fig.autofmt_xdate()
...
def vitals_graphs_index(request, id):
    return render(request, 'popup_graphs_index.html', {
        'id': id,
    })
