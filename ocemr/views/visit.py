...
def visit_claim(request, id):
    ...
    try:
        old_v = Visit.objects.filter(patient=p).filter(
            status='RESO').order_by('-finishedDateTime')[0]
    except:
        old_v = None
    if old_v != None:
        ...
        for old_diag in old_diags:
            try:  # Check for existing Diagnosis in this record
                d = Diagnosis.objects.get(
                    type=old_diag.type, patient=p, visit=v)
            except:
                d = None
...
def visit_subj_new(request, id, symptomtypeid):
    ...
    if request.method == 'POST':  # If the form has been submitted...
        form = NewVisitSymptomForm(
            vid, stid, request.POST)  # A form bound to the POST data
...
def visit_obje(request, id):
    ...
    vital_times_in = Vital.objects.filter(
        visit=v).values('observedDateTime').distinct()
    vital_times = []
    for vt in vital_times_in:
        vitals = Vital.objects.filter(
            visit=v, observedDateTime=vt['observedDateTime'])
...
def visit_obje_vitals_new(request, id):
    ...
    if request.method == 'POST':  # If the form has been submitted...
        form = NewVitalsForm(v, request.user,
...
def visit_obje_examNote_new(request, id, examnotetypeid):
    ...
    if request.method == 'POST':  # If the form has been submitted...
        form = NewExamNoteForm(v, ent, request.user,
                               request.POST)  # A form bound to the POST data
        if form.is_valid():  # All validation rules pass
            o = form.save()
            return HttpResponseRedirect('/close_window/')
...
def visit_obje_examNote_edit(request, id, examnoteid):
    ...
    return render(
        request, 'popup_form.html', {
            ...
            'form_action':
            '/visit/%s/obje/examNote/edit/%s/' % (id, examnoteid),
            'form': form,
        })
...
def visit_plan_diag_new(request, id):
    ...
    if request.method == 'POST':
        form = NewDiagnosisForm(v, request.user,
                                request.POST)  # A form bound to the POST data
        if form.is_valid():  # All validation rules pass
            o = form.save()
            return HttpResponseRedirect('/close_window/')
...
def visit_meds_new(request, id, did):
    ...
    if request.method == 'POST':
        form = NewMedForm(d, request.user,
                          request.POST)  # A form bound to the POST data
        if form.is_valid():  # All validation rules pass
            o = form.save()
            return HttpResponseRedirect('/close_window/')
...
def visit_refe_new(request, id):
    ...
    if request.method == 'POST':  # If the form has been submitted...
        form = NewReferralForm(v, request.user,
                               request.POST)  # A form bound to the POST data
        if form.is_valid():  # All validation rules pass
            o = form.save()
            return HttpResponseRedirect('/close_window/')
...
def visit_allergy_new(request, id):
    ...
    if request.method == 'POST':  # If the form has been submitted...
        form = NewAllergyForm(v, request.user,
                              request.POST)  # A form bound to the POST data
        if form.is_valid():  # All validation rules pass
            o = form.save()
            return HttpResponseRedirect('/close_window/')
...
def visit_collect(request, id):
    ...
    if request.method == 'POST':  # If the form has been submitted...
        form = NewCashLogForm(v, request.user,
                              request.POST)  # A form bound to the POST data
        if form.is_valid():  # All validation rules pass
            o = form.save()
            return HttpResponseRedirect('/close_window/')
...
def visit_cost_estimate_detail(request, id):
    ...
    for row in ced:
        table += "<TR><TD>%s<TD>%s<TD>%s<TD>%s</TR>" % (row[0], row[1], row[2],
                                                        row[3])
...
def visit_bill_amount(request, id):
    ...
    if request.method == 'POST':  # If the form has been submitted...
        form = EditBillAmountForm(
            v.cost, request.POST)  # A form bound to the POST data
        if form.is_valid():  # All validation rules pass
...
def visit_record(request, id, type):
    ...
    head_text += "\n\n\tPatient: %s\tVisit# %05d\tDate: %02d-%02d-%02d\n" % (
        v.patient, v.id, d.day, d.month, d.year)
    allergy_list = []
    ...
    if type == "print":
        ...
        cmd = " ".join(('enscript', '-P', PRINTER_NAME, '--word-wrap',
                        '--mark-wrapped-lines=arrow', '--font=Times-Roman12',
                        '--header=', '--media=' + PAPER_SIZE))
        p = Popen(
            cmd,
            shell=True,
            bufsize=0,
