...
def med_queue(request):
    ...
    q_inactive = (Q(resolvedDateTime__gte=dt_start) &
                  Q(resolvedDateTime__lte=dt_end)) & Q(status='RESO')

    from ocemr.models import Visit
    visits_active = Visit.objects.filter(q_active).order_by('finishedDateTime')
    visits_inactive = Visit.objects.filter(q_inactive).order_by(
        '-resolvedDateTime')
...
def med_substitute(request, id):
    ...
    return HttpResponseRedirect(
        '/visit/%s/meds/new/%s/' % (m.visit.id, m.diagnosis.id))
...
def med_notate(request, id):
    ...
    if request.method == 'POST':  # If the form has been submitted...
        form = NewMedNoteForm(m, request.user,
    ...
    if request.user != m.addedBy:
        return render(
            request, 'popup_info.html', {
                'title': 'Error',
                'info':
                'You cannot edit This Med, cancel it and re-add instead.',
            })
