...
class NewScheduledVisitForm(forms.ModelForm):
    ...
    scheduledDate = forms.DateField(
        required=False, widget=widgets.CalendarWidget)
    patient = forms.ModelChoiceField(
        queryset=Patient.objects.all(), widget=forms.HiddenInput)
...
class NewVitalsForm(forms.Form):
    ...
    observedBy = forms.ModelChoiceField(
        queryset=User.objects.all(), widget=forms.HiddenInput)
    observedDateTime = forms.DateTimeField(
        widget=forms.HiddenInput, required=False)

    temp_in = forms.CharField(
        label="Temp",
        help_text=
        "in celcius or fahrenheit. (valid examples \"98.7f\" or \"37 c\".)",
        required=False)
    ...
    height_in = forms.CharField(
        label="Height",
        help_text=
        "in centimeters or inches. (valid examples \"177 cm\" or \"70.25in\".)",
        required=False)
    weight_in = forms.CharField(
        label="Weight",
        help_text=
        "in kilograms or pounds. (valid examples \"77.5 kg\" or \"150lb\".)",
        required=False)
...
class NewPregnancyForm(forms.Form):
    ...
    addedBy = forms.ModelChoiceField(
        queryset=User.objects.all(), widget=forms.HiddenInput)
    addedDateTime = forms.DateTimeField(
        widget=forms.HiddenInput, required=False)
...
class NewLabNoteForm(forms.ModelForm):
    ...
    lab = forms.ModelChoiceField(
        queryset=Lab.objects.all(), widget=forms.HiddenInput)
    addedBy = forms.ModelChoiceField(
        queryset=User.objects.all(), widget=forms.HiddenInput)
...
class NewMedNoteForm(forms.ModelForm):
    ...
    med = forms.ModelChoiceField(
        queryset=Med.objects.all(), widget=forms.HiddenInput)
    addedBy = forms.ModelChoiceField(
        queryset=User.objects.all(), widget=forms.HiddenInput)
