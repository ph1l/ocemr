...
def post_syncdb_auto_upgrade(sender, **kwargs):
    ...
    re_match_filename_sql = re.compile(
        "^(\d+)-(\d+)-%s.sql$" % (DATABASE_ENGINE))
    ...
    for fname in sorted(os.listdir("%s/schema_updates" % CONTRIB_PATH)):
        ...
        if m:
            ...
            if DEBUG:
                print "found %s: major=%d, minor=%d" % (fname, f_major,
                                                        f_minor)
            if f_major > latest_version.major or (
                    f_major == latest_version.major
                    and f_minor > latest_version.minor):
    ...
    if biggest_major > latest_version.major or (
            biggest_major == latest_version.major
            and biggest_minor > latest_version.minor):
