...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if infile[-4:] == ".bz2":
            outfile = infile[:-4]
            self.stdout.write(
                "Decompressing \"%s\" to \"%s\"..." % (infile, outfile))
            self.do_decompress_backup(infile, outfile)
            infile = outfile
        elif infile[-4:] == ".gpg":
            outfile = infile[:-4]
            self.stdout.write(
                "Decrypting \"%s\" to \"%s\" ..." % (infile, outfile))
        ...
        if infile[-6:] == ".mysql":
            if self.engine != 'mysql':
                raise Exception(
                    "Backup from MySQL, but current engine is not!")
            self.stdout.write('Doing MySQL restore to database %s from %s...' %
                              (self.db, infile))
            self.do_mysql_restore(infile)
        elif infile[-8:] == ".sqlite3":
            if self.engine != 'sqlite3':
                raise Exception(
                    "Backup from SqLite3, but current engine is not!")
            self.stdout.write('Doing sqlite3 restore to database %s from %s' %
                              (self.db, infile))
	...
	"""
        ...
        if exit_status != 0:
            raise Exception(
                "Decrypt command (%s) failed with %s." % (cmd, exit_status))
	...
	"""
        ...
        if exit_status != 0:
            raise Exception(
                "restore command (%s) failed with %s." % (cmd, exit_status))
